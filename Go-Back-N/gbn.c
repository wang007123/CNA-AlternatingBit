...
void A_output(struct msg message)
{
  ...
  if ( windowcount < WINDOWSIZE) {
    ...
    /*the first packet the windows have been sent */
    if(A_nextseqnum == 0)
    {
      starttimer(A,RTT);
    }
...
void A_input(struct pkt packet)
{
  ...
  if (!IsCorrupted(packet)) {
    ...
    if (packet.acknum >= true_ack) {    /**** 2. FILL IN CODE replace TRUE with test whether this is a new ACK ***/
      ...
      forward= packet.acknum - true_ack +1;

      true_ack = true_ack + forward;
      /* delete the acked packets from window buffer */
      windowcount = windowcount - forward;
      ...
      if(windowcount != 0)
      	starttimer(A,RTT);
      windowfirst= (packet.acknum+1)%WINDOWSIZE;
    }else
    {
    	if (TRACE > 0)
...
void A_timerinterrupt(void)
{
  ...
  /*resend all the data  store in the window*/
    if(re == windowlast)
    {
